...
// AABB => Now we will check the collision between two objects (the ball and padlle)
...
function loop() {
    ...
    // Preventing the paddles from going through the walls 
    ...
    // Resetting the ball if it goes past padd (but only if we have not already done so)
    if ((ball.x < 0 || ball.x > canvas.width) && !ball.resetting) {
        ball.resetting = true;

        // give some time for player to recover before launching the ball again
        // for that, we are going to use setTimeout function
        setTimeout(() => {
    ...
    // Check to see if ball collides with padlle
